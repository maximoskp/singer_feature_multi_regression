...
result_fields = ['epoch', 'train_loss', 'train_r_loss', 'train_c_loss', 'train_acc', 'val_loss', 'val_r_loss', 'val_c_loss', 'val_acc']
...
for epoch in range(epochs):
    ...
    with tqdm(train_loader, unit='batch') as tepoch:
        ...
        for b in tepoch:
            ...
            # update loss
            batch_num += 1
            running_loss += y.loss['regressors'].item() + y.loss['classifier'].item()
            running_r_loss += y.loss['regressors'].item()
            running_c_loss += y.loss['classifier'].item()
            train_loss = running_loss/batch_num
            train_r_loss = running_r_loss/batch_num
            train_c_loss = running_c_loss/batch_num
            # accuracy
    ...
    with torch.no_grad():
        ...
        with tqdm(test_loader, unit='batch') as tepoch:
            ...
            for b in tepoch:
                ...
                val_r_loss = running_r_loss/batch_num
                val_c_loss = running_c_loss/batch_num
                # accuracy
                prediction = y['logits']['singer_id'][0].argmax(dim=1, keepdim=True).squeeze()
                running_accuracy += (prediction == torch.Tensor(b[1]['singer_id']).to(model.dev)).sum().item()/len(b[1]['singer_id'])
            ...
            with open( results_path, 'a' ) as f:
                writer = csv.writer(f)
                writer.writerow( [epoch, train_loss, train_r_loss, train_c_loss, train_accuracy, val_loss, val_r_loss, val_c_loss, val_accuracy] )